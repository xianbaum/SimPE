...
namespace SimPe.PackedFiles.UserInterface
{
	...
	public class StrForm : System.Windows.Forms.Form, IPackedFileUI
	{
		...
		private Hashtable lngs = new Hashtable();
		private Hashtable strs = new Hashtable();

		private void StrClear(int i)
		{
			...
			if (lid > 1) 
			{
				this.lvStrItems.Items[i].SubItems[2].Text = "";
			}
		}

		private void StrDelete(int i)
		{
			internalchg = true;

			for (byte j = 1; j < 44; j++)
			{
				for (int ix = i; ix < count - 1; ix++)
				{
		...
		private void checkLang(byte l)
		{
			StrItem[] aSI = wrapper[l];
			bool empty = true;
			for (int i = aSI.Length - 1; i > 0 && empty; i--)
				if (aSI[i].Title.Trim().Length + aSI[i].Description.Trim().Length > 0) empty = false;
			if (empty) lngs.Remove(l); else lngs[l] = true;
			this.cbLngSelect.Items[l - 1] = ((SimPe.Data.MetaData.Languages)l).ToString() + (empty ? " (empty)" : "");

			this.btnLngClear.Enabled = (lngs[lid] != null);
			this.btnClearAll.Enabled = lngs.Count > 1;
		}

		private void checkStr(int i)
		{
			bool present = false;
			for (byte l = 2; l < 44 && !present; l++)
			{
				StrItem si = wrapper[l, i];
				if (si != null && si.Title.Trim().Length + si.Description.Trim().Length > 0) present = true;
			}
			if (present) strs[i] = true; else strs.Remove(i);

			this.btnStrClear.Enabled = (strs[index] != null);
		}
		...
		private void updateLists()
		{
			...
			{
				this.lvStrItems.Items.Add(new ListViewItem(new string[] {
																			"0x" + Helper.HexString((ushort)i),
																			wrapper[1, i].Title,
																			(wrapper[lid, i] != null) ? wrapper[lid, i].Title : ""
																		}));
			}
			...
			if (this.lvStrItems.Items.Count > 0)
				this.lvStrItems.Items[index].Selected = true;
		...
		private void setLid(byte l)
		{
			...
			for (int i = 0; i < count; i++) this.lvStrItems.Items[i].SubItems[2].Text = (wrapper[lid, i] != null) ? wrapper[lid, i].Title : "";
			this.btnLngClear.Text = "Clear " + ((SimPe.Data.MetaData.Languages)lid).ToString();
			this.btnLngClear.Enabled = (lid != 1) && !((string)(this.cbLngSelect.Items[lid - 1])).EndsWith(" (empty)");
			displayStrItem();

			this.btnLngPrev.Enabled = (lid > 0);
			this.btnLngNext.Enabled = (this.cbLngSelect.Items.Count > 0) && (lid < 43);
		}

		private void clearLid()
		{
			this.btnLngClear.Text = "Clear Lang";
			this.btnLngPrev.Enabled = this.btnLngNext.Enabled = this.btnLngClear.Enabled = false;
			setIndex(-1);
		}
		...
		private void clearIndex() { setIndex(-1); }
		...
		private void hex16_TextChanged(object sender, System.EventArgs ev)
		{
			...
			{
				case 0: internalchg = true; wrapper.Format = val; internalchg = false; break;
			}
		}
		...
		{
			if (this.cbLngSelect.SelectedIndex < 1) return;
		...
		private void btnStrAdd_Click(object sender, System.EventArgs e)
		{
			...
			this.lvStrItems.Items[count - 1].Selected = true;
			this.cbLngSelect.SelectedIndex = -1;
