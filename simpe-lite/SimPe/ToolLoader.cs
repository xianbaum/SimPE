...
namespace SimPe
{
	...
	/// </summary>
	public class ToolLoaderListBoxItem : ToolLoaderItem 
	{
		public ToolLoaderListBoxItem(ToolLoaderItem tli) : base (tli.Name) 
		{
			name = tli.Name;
	...
	/// </summary>
	public class ToolLoaderItem 
	{
		/// <summary>
		/// Creates a new Instance
		/// </summary>
		/// <param name="name">Name of this Tool</param>
		public ToolLoaderItem(string name)
		{
			this.name = name;
			filename = "";
			arguments = "{tempfile}";
		...
		public static bool OpenPackedFile(string filename, ref Packages.PackedFileDescriptor pfd, Interfaces.Files.IPackageFile package) 
		{			
			try 
			{
				try 
		...
		public void Execute(Packages.PackedFileDescriptor pfd, SimPe.Packages.GeneratableFile package) 
		{
			...
			try 
			{
				SavePackedFile(extfile, false, pfd, package);
				...
				OpenPackedFile(extfile, ref pfd, package);
				pfd.Filename = null;
				pfd.Path = null;
			} 
			finally 
		...
		{
			RegistryKey rk = Helper.WindowsRegistry.RegistryKey.CreateSubKey("ExtTools");
	...
	{
		static ToolLoaderItem[] items;

		/// <summary>
		/// List of all available Items
		/// </summary>
		public static ToolLoaderItem[] Items 
		{
		...
		public static void Add(ToolLoaderItem tli) 
		{
			if (items==null) Load();
			items = (ToolLoaderItem[])Helper.Add(items, tli);
		}
		...
		/// </summary>
		/// <param name="tli"></param>
		public static void Remove(ToolLoaderItem tli) 
		{
			if (items==null) Load();
			items = (ToolLoaderItem[])Helper.Delete(items, tli);
		}
		...
		public static void StoreTools()
		{
			...
			{
				Helper.WindowsRegistry.RegistryKey.CreateSubKey("ExtTools").DeleteSubKey(name);
			}
		...
		protected static void Load() 
		{
			...
			foreach (string name in names) 
			{
				string rname = ToolLoaderItem.SplitName(name);
				RegistryKey srk = rk.CreateSubKey(name);

				ToolLoaderItem tli = new ToolLoaderItem(rname);
			...
			items = new ToolLoaderItem[list.Count];
			list.CopyTo(items);
		}
		...
		/// <returns></returns>
		public static ToolLoaderItem[] UsableItems(uint type) 
		{
			if (items==null) Load();

			ArrayList list = new ArrayList();
			if (type!=0xffffffff) foreach (ToolLoaderItem tli in items) if (tli.Type == type) list.Add(tli);
			foreach (ToolLoaderItem tli in items) if (tli.Type == 0xffffffff) list.Add(tli);

			ToolLoaderItem[] ret = new ToolLoaderItem[list.Count];
			list.CopyTo(ret);
