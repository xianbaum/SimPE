	...
	internal class ImportGmdcGroupsForm : System.Windows.Forms.Form
	{
		...
		public static ImportOptions Execute(SimPe.Plugin.GeometryDataContainer gmdc, ImportedGroups actions, ImportedBones joints)
		{
			...
			if (toobig) 			
			{
                ...
                {
                    Helper.ExceptionMessage(new Warning("One or more of the imported Mesh Groups contain too many Vertices or Faces", "SimPe is running in Debug Mode so the maximum Number of Vertices is set to " + SimPe.Plugin.Gmdc.AbstractGmdcImporter.CRITICAL_VERTEX_AMOUNT.ToString() + " and the maximum amount of Faces to " + SimPe.Plugin.Gmdc.AbstractGmdcImporter.CRITICAL_FACE_AMOUNT.ToString() + ".\n\nSimPe can Import this Mesh when not testing"));
