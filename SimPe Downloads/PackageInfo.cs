...
namespace SimPe.Plugin.Downloads
{
	...
	public class PackageInfo : Downloads.IPackageInfo	, System.IDisposable
	{
		...
		/// <returns>The expansion which makes this File available (according to the FileTable, <see cref="Expansion.Custom"/> marks a Custom File from the Downloads Folder)</returns>
		...
		public bool HighVertexCount
		{
			get
            {
                if (booby.PrettyGirls.IsTitsInstalled() || booby.PrettyGirls.IsAngelsInstalled() || Helper.WindowsRegistry.CreatorMode) return (VertexCount > 24000);
                return (VertexCount > 2000); // high for dum dums
            }
		}
		...
		public bool HighFaceCount
		{
			get
            {
                if (booby.PrettyGirls.IsTitsInstalled() || booby.PrettyGirls.IsAngelsInstalled() || Helper.WindowsRegistry.CreatorMode) return (FaceCount > 36000);
                return (FaceCount > 5000); // was VertexCount not FaceCount ?
            }
		}
