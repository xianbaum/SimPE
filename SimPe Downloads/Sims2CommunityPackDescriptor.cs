...
namespace SimPe.Packages
{
	...
	public class S2CPDescriptorBase
	{
		...
		public static string GetGlobalGuid(File p, ref string name, ref string title, ref string author, ref string contact, ref string description, ref string gameguid) 
		{
			...
			if (pfd!=null) 
			{
				SimPe.PackedFiles.Wrapper.Str str = new SimPe.PackedFiles.Wrapper.Str();
				str.ProcessData(pfd, p);
                SimPe.PackedFiles.Wrapper.StrItemList sitems = str.FallbackedLanguageItems(Helper.WindowsRegistry.LanguageCode);

				if (sitems.Length>0) title = sitems[0].Title;
				if (sitems.Length>1) description = sitems[1].Title;
                if (sitems.Length > 2) title += " " + sitems[2].Title;
			}
			return guid;
		}
	...
	public class S2CPDescriptor : S2CPDescriptorBase
	{
			...
			: base (p)
		{
            ...
            if (p != null && extension) guid = GetSetGlobalGuid(p, ref name, ref this.title, ref this.author, ref this.contact, ref this.description, ref gameguid);
		}
		...
		public override ValidationState Valid
		{
			get 
			{
                ...
                if (n != name) return ValidationState.NameMismatch;
                if (g != guid) return ValidationState.GlobalGUIDMismatch;
                if (a != author && a != "" && author != "") return ValidationState.AuthorMismatch;
