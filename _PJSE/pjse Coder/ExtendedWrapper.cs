...
namespace pjse
{
		...
		, IMultiplePackedFileWrapper	//Allow Multiple Instances
	{
		...
		public uint PrivateGroup
        {
            get
            {
                if (Context == Scope.Global || Context == Scope.SemiGlobal || this.FileDescriptor == null)
                    return 0;
		...
		public uint SemiGroup
		{
			get
			{
                if (Context == Scope.Global || this.FileDescriptor == null)
                    return 0;
        ...
        public SimPe.Interfaces.Plugin.Internal.IPackedFileWrapper SiblingResource(uint type)
        {
            ...
            if (wrp != null) wrp.ProcessData(items[0].PFD, items[0].Package);
